...
Desc: Basic script to read from a file and write some selected text into another file. 
Author: Avik Deb
...
sub main() {
	...
	my $output = 'C:\Avik\perlworkspace\perl\output2.txt';
	...
	print "Successfully written texts in $output.\n";
	close(INPUT);
	close(OUTPUT);
}
